...
import com.saivamsi.jwtTemplate.model.Token;
import com.saivamsi.jwtTemplate.repository.TokenRepository;
...
public class AuthenticationService {
    ...
    public AuthenticationResponse registerUser(String username, String email, String password) {
        ...
        if (existingUser.isPresent()) {
            if (existingUser.get().getEmail().equals(email.toLowerCase())) {
                throw new GlobalException(HttpStatus.CONFLICT, Error.builder().subject("email").message("email in use").build());
            }
            if (existingUser.get().getUsername().equals(username.toLowerCase())) {
                throw new GlobalException(HttpStatus.CONFLICT, Error.builder().subject("username").message("username in use").build());
            }
        }
    ...
    public AuthenticationResponse loginUser(String principal, String password) {
        try {
            ...
            throw new GlobalException(HttpStatus.FORBIDDEN, Error.builder().subject("emailOrUsername").message("invalid login").build());
        }
    }
    ...
    public AuthenticationResponse refreshToken(String refreshToken) {
        try {
            ...
            throw new GlobalException(HttpStatus.FORBIDDEN, Error.builder().subject("account").message("unable to refresh token").build());
        }
